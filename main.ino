...
//#include "CommanderSS.h" //debugging only
#include <ax12.h>
...
//#define ENABLE_COMMANDER 1 //debugging only
...
#define DEBUG_SERIAL 1 //debugging only
//#endif
...
int samplingInterval = 10;          // how often to run the main loop (in ms)
...
void dynamixelSynchMoveSetup(byte servo, int pos, int speed) {
  ...
  totalSynchServos++;
#ifdef DEBUG_SERIAL
  debugSerial.print("SYNCH COMMAND. num servos: ");
  debugSerial.println(totalSynchServos);
#endif
}
...
#ifdef DEBUG_SERIAL
        debugSerial.print("T,");
        debugSerial.print(millis() - resetTime);
        debugSerial.print(",");
...
void setup()
{
  ...
  command.begin(38400);
  delay(100);
  debugSerial.println("COMMANDER ENABLED");
  ...
  debugSerial.println("NO COMMANDER, JUST DEBUG.");
  debugSerial.println("Setup finished");
  debugSerial.print("Firmata Version: ");
  debugSerial.print(FIRMATA_MAJOR_VERSION); debugSerial.print(".");
  debugSerial.print(FIRMATA_MINOR_VERSION); debugSerial.print(".");
...
void loop()
{
  ...
  if (currentMillis - previousMillis > samplingInterval) {
    ...
    for (pin = 0; pin < TOTAL_PINS; pin++) { //debugged
      ...
      if (IS_PIN_ANALOG(pin) && pinConfig[pin] == ANALOG) {
        ...
        if (analogInputsToReport & (1 << analogPin)) {
          ...
          if (blocks) { 
            ...
            widthAnalogReading = pixy.blocks[j].width;
            heightAnalogReading = pixy.blocks[j].height;

            if (analogPin == 0) {
              Firmata.sendAnalog (analogPin, analogReading);
              debugSerial.print("Number on analog pin: ");
              debugSerial.print(analogPin);
              debugSerial.print(" X Reading: ");
              debugSerial.println(analogReading);
            } else if (analogPin == 1) {
              Firmata.sendAnalog (analogPin, yAnalogReading);
              debugSerial.print("Number on analog pin: ");
              debugSerial.print(analogPin);
              debugSerial.print(" Y Reading: ");
              debugSerial.println(yAnalogReading);
            } else if (analogPin == 2) {
              Firmata.sendAnalog (analogPin, widthAnalogReading);
              debugSerial.print("Number on analog pin: ");
              debugSerial.print(analogPin);
              debugSerial.print(" Width reading: ");
              debugSerial.println(widthAnalogReading);
            } else if (analogPin == 3) {
              Firmata.sendAnalog (analogPin, heightAnalogReading);
              debugSerial.print("Number on analog pin: ");
              debugSerial.print(analogPin);
              debugSerial.print(" Height reading: ");
              debugSerial.println(heightAnalogReading);
            }
          } else {
            analogReading = 0;
            yAnalogReading = 0;
            widthAnalogReading = 0;
            heightAnalogReading = 0;
